...
所有自动发送的固定回复、AI 回复和工具跟进回复都会默认在结尾加上 ` ar` 标记，用来和用户真人回复区分。这个标记也会被智能上下文和风格学习脚本识别并跳过，避免把自动回复当成真人样本学习。如果确实要改标记，可以设置 `LARK_AUTOREPLY_MARKER`，例如 `ᵃʳ`。历史 `AR:` 前缀仍会被识别为自动回复，避免污染训练样本。
